...
class ZoomR16Transport:
    """Representing the transport section of the ZoomR16 Control: """
    ...
    def destroy(self):
        ...
        ZoomR16ControlComponent.destroy(self)
    ...
    def handle_jog_wheel_rotation(self, value):
        ...
        if self.control_is_pressed():
            ...
            ## Zoom if ALT key pressed
            if self.is_alt_pressed():
                nav = Live.Application.Application.View.NavDirection
                if backwards:
                    self.application().view.zoom_view(nav.left, '', True)
                else:
                    self.application().view.zoom_view(nav.right, '', True)
            else:
        ...
        if switch_id == SID_JOG_CURSOR_UP:
            ...
            if value == BUTTON_PRESSED:
                if self.session_is_visible():
                    ...
                    #self.is_alt_pressed() = not self.is_alt_pressed()
                    self.__update_zoom_button_led()
    ...
    def __on_cursor_up_pressed(self):
        ...
        nav = Live.Application.Application.View.NavDirection
        if self.is_alt_pressed():
            self.application().view.zoom_view(nav.up, '', self.is_alt_pressed())
        else:
            self.application().view.scroll_view(nav.up, '', self.is_alt_pressed())

    def __on_cursor_down_pressed(self):
        
        nav = Live.Application.Application.View.NavDirection
        if self.is_alt_pressed():
            self.application().view.zoom_view(nav.down, '', self.is_alt_pressed())
        else:
            self.application().view.scroll_view(nav.down, '', self.is_alt_pressed())

    def __on_cursor_left_pressed(self):

        nav = Live.Application.Application.View.NavDirection
        if self.is_alt_pressed():
            self.application().view.zoom_view(nav.left, '', self.is_alt_pressed())
        else:
            self.application().view.scroll_view(nav.left, '', self.is_alt_pressed())

    def __on_cursor_right_pressed(self):
        nav = Live.Application.Application.View.NavDirection
        if self.is_alt_pressed():
            self.application().view.zoom_view(nav.right, '', self.is_alt_pressed())
        else:
            self.application().view.scroll_view(nav.right, '', self.is_alt_pressed())
    ...
    def application(self):
        return self.__controller.application()
